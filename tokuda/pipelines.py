...
class TokudaPipeline:

    # Database setup
    conn = sqlite3.connect('tokuda.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `tokuda`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM tokuda WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO tokuda (date, title, link, content)"
